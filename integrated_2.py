...
def product_error(a,b,std_a,std_b):
    product = a*b
    error = a*b*(np.sqrt((std_a/a)**2 + (std_b/b)**2))
    return product,error

def product_error_sqrt(a,b,std_a,std_b):
    product = np.sqrt(a*b)
    error = product*(np.sqrt((std_a/(2*a))**2 + (std_b/(2*b))**2))
    return product,error
...
def fit_ellipse_from_gradient_2(filtered_image, mask, center, radius):
    # Compute gradient magnitude with a lower sigma for more sensitivity
    gradient_mag = gaussian_gradient_magnitude(filtered_image, sigma=1)

    # Use a more conservative threshold
    threshold = gradient_mag.mean()
    ...
    binary_image = remove_small_objects(binary_image, min_size=10)  # Adjust min_size as needed
...
def save_images(directory_path, output_directory,results_directory):
    ...
    for file in files:
        ...
        #Extract current
        #current = np.float(file.split('_')[0])
        #print(current)
        # Extract the screen name
        screen = file.split('_')[1]
        print(screen)
        ...
        with h5py.File(file_path, 'r') as h5file:
            ...
            current_all = h5file['images'].attrs['AWA:Drive:DS1:RB'][:]
            print(current_all)
            current_mean = np.mean(current_all)
            current_std = np.std(current_all)
            ...
            for i, image in enumerate(images):
                ...
                # Fit an ellipse to the particles using gradient magnitude for Yag6 and regular fit for Yag7
                if screen == 'Yag6':
                    result = fit_ellipse_from_gradient_2(filtered_image, mask, center, radius)
                elif screen == 'Yag7':
            ...
            if results:
                ...
                ax_main.imshow(plot_image, cmap='viridis')
                ...
                result_data = {
                    'File': file,
                    'Magnetic field': current_mean*0.0254/100,
                    'Std Magnetic field': current_std * 0.0254 / 100,
                    'Centroid x': avg_xc,
                    'Centroid y': avg_yc,
                    ...
                    'Circular Beam Radius': product_error_sqrt(avg_major_axis_length_mm,avg_minor_axis_length_mm,std_major_axis_length_mm,std_minor_axis_length_mm)[0],
                    'Circular beam radius error' : product_error_sqrt(avg_major_axis_length_mm,avg_minor_axis_length_mm,std_major_axis_length_mm,std_minor_axis_length_mm)[1]
                }
                ...
                # Save the plot
                output_path = os.path.join(output_directory, f'{current_mean}_{screen}_position_{position}.png')
